...
SAMPLE run:
>> python run_center_candidate_prediction.py -list none \
...
def load_compute_detect_centers(idx_row, params, classif=None, path_classif='',
    ...
    try:
        ...
        dict_detect = run_train.detect_center_candidates(name, img, segm, None, slic,
                                                         points, features, feature_names,
...
def main_detect(params):
    ...
    tl_expt.create_subfolders(params['path_output'], LIST_SUBFOLDER)
    ...
    if params['nb_jobs'] > 1:
        ...
        for dict_center in mproc_pool.imap_unordered(wrapper_detection,
            ...
            df_stat.to_csv(os.path.join(params['path_output'], NAME_CSV_TRIPLES_TEMP))
            tqdm_bar.update()
        mproc_pool.close()
        mproc_pool.join()
    else:
    ...
    df_stat.to_csv(os.path.join(params['path_output'], NAME_CSV_TRIPLES))
