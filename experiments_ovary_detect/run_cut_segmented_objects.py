...
def arg_parse_params(dict_paths):
    ...
    for k in dict_paths:
        if not os.path.exists(os.path.dirname(dict_paths[k])):
...
def export_cut_objects(df_row, path_out, padding, use_mask=True, bg_color=None):
    ...
    if annot.shape[:2] != img.shape[:2]:
        raise AssertionError('image sizes not match %r vs %r' % (annot.shape, img.shape))
...
def main(dict_paths, padding=0, use_mask=False, bg_color=None, nb_workers=NB_WORKERS):
    ...
    if not os.path.isdir(dict_paths['output']):
        if not os.path.isdir(os.path.dirname(dict_paths['output'])):
            raise AssertionError('"%s" should be folder' % dict_paths['output'])
