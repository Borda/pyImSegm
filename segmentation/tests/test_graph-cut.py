...
class TestGraphCut(unittest.TestCase):
    ...
    def test_count_transitions_segment(self):
        ...
        slic = tl_spx.segment_slic_img2d(img, sp_size=15, rltv_compact=0.2)
        label_hist = tl_lb.histogram_regions_labels_norm(slic, annot)
        labels = np.argmax(label_hist, axis=1)
        trans = tl_gc.count_label_transitions_connected_segments({'a': slic},
                                                                 {'a': labels})
        path_csv = os.path.join(PATH_OUTPUT, 'labels_transitions.csv')
        pd.DataFrame(trans).to_csv(path_csv)
        gc_regul = tl_gc.compute_pairwise_cost_from_transitions(trans, 10.)

        np.random.seed(0)
        features = np.tile(labels, (5, 1)).T.astype(float)
        features += np.random.random(features.shape) - 0.5

        gmm = tl_gc.estim_class_model_gmm(features, 4)
        proba = gmm.predict_proba(features)

        tl_gc.segment_graph_cut_general(slic, proba, gc_regul)
