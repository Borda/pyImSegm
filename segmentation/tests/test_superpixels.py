...
class TestSuperpixels(unittest.TestCase):
    ...
    def test_segm_connect(self):
        ...
        vertices, edges = tl_spx.make_graph_segm_connect2d_conn4(self.seg2d)
        logging.debug('vertices: {} -> edges: {}'.format(vertices, edges))

        logging.debug(self.seg3d)
        vertices, edges = tl_spx.make_graph_segm_connect3d_conn6(self.seg3d)
        logging.debug('vertices: {} -> edges: {}'.format(vertices, edges))

    def test_general(self):
        slic = tl_spx.segment_slic_img2d(self.img, sp_size=15, rltv_compact=0.2)
