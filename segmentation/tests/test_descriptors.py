...
def draw_ray_results(seg, center, points, ray_dist_raw, ray_dist, name):
...
class TestFeatures(unittest.TestCase):
    ...
    def test_features_rgb(self):
        ...
        start = time.time()
        f = tl_fts.cython_img2d_color_mean(im, seg)
    ...
    def test_filter_banks(self, ax_size=SUBPLOT_SIZE_FILTER_BANK):
        filters, names = tl_fts.create_filter_bank_lm_2d()
    ...
    def test_ray_features_circle(self):
        ...
        for i, point in enumerate([(200, 250), (150, 200), (250, 200), (250, 300)]):
            ray_dist_raw = tl_fts.compute_ray_features_segm_2d(
                                    seg, point, angle_step=ANGULAR_STEP)
            ray_dist, shift = tl_fts.shift_ray_features(ray_dist_raw)
            points = tl_fts.reconstruct_ray_features_2d(point, ray_dist, shift)
            draw_ray_results(seg, point, points, ray_dist_raw, ray_dist,
                             'circle-%i.png' % i)
    ...
    def test_ray_features_ellipse(self):
        ...
        for i, point in enumerate([(200, 250), (150, 200), (250, 300)]):
            ray_dist_raw = tl_fts.compute_ray_features_segm_2d(
                                    seg, point, angle_step=ANGULAR_STEP)
            # ray_dist, shift = tl_fts.shift_ray_features(ray_dist_raw)
            points = tl_fts.reconstruct_ray_features_2d(point, ray_dist_raw)
            draw_ray_results(seg, point, points, ray_dist_raw, [],
                             'ellipse-%i.png' % i)
    ...
    def test_ray_features_circle_down_edge(self):
        ...
        seg[x, y] = True
        list_points = [(200, 250), (150, 200), (250, 200), (250, 300)]
    ...
    def test_show_image_features_clr2d(self):
        ...
        slic = tl_spx.segment_slic_img2d(img, sp_size=10,
                                         rltv_compact=0.2)

        features, names = tl_fts.compute_selected_features_color2d(
                                         img, slic, tl_fts.FEATURES_SET_ALL)
