...
def getColourMeanImg2dRGB(np.ndarray[np.int16_t, ndim=3] img,
    ...
    cdef:
        ...
        np.ndarray[np.float64_t, ndim=2] features = np.zeros([nbSegments, 3], dtype=np.float64)
        np.ndarray[np.int32_t, ndim=2] count = np.zeros([nbSegments, 1], dtype=np.int32)
        int w = seg.shape[0]
        int h = seg.shape[1]
        int x, y, i, id
    for x in range(w):
...
# def computeColorImage2dMean(float[:,:,:] img, int[:,:] seg):
#     cdef:
#         int nbSegments = np.max(seg) +1
#         float[:,:] features = np.zeros([nbSegments, 3], dtype=np.float32)
#         int[:] count = np.zeros(nbSegments, dtype=np.int32)
#         int w = seg.shape[0]
#         int h = seg.shape[1]
#         int z, x, y, i, id
...
def normColorFeatures(int[:, :] seg, float[:, :] features):
    cdef:
...
def computeColorImage2dMean(float[:, :, :] img, int[:, :] seg):
    cdef:
        int nb_segments = np.max(seg) + 1
        float[:, :] features = np.zeros([nb_segments, 3], dtype=np.float32)
        int w = seg.shape[0]
        int h = seg.shape[1]
        int z, x, y, i, id
    for z in prange(3, nogil=True):
...
def computeColorImage2dEnergy(float[:, :, :] img, int[:, :] seg):
    cdef:
        int nb_segments = np.max(seg) + 1
        float[:, :] features = np.zeros([nb_segments, 3], dtype=np.float32)
        float val
...
def computeColorImage2dVariance(float[:, :, :] img, int[:, :] seg, float[:, :] mean):
    cdef:
        int nb_segments = np.max(seg) + 1
        float[:, :] features = np.zeros([nb_segments, 3], dtype=np.float32)
        int w = seg.shape[0]
...
def computeGrayImage3dMean(float[:, :, :] img, int[:, :, :] seg):
    cdef:
        int nb_segments = np.max(seg) + 1
        float[:] features = np.zeros(nb_segments, dtype=np.float32)
        int[:] count = np.zeros(nb_segments, dtype=np.int32)
        # np.ndarray[np.float64_t, ndim=2] features = np.zeros([nbSegments, 1], dtype=np.float64)
        # np.ndarray[np.int32_t, ndim=2] count = np.zeros([nbSegments, 1], dtype=np.int32)
        int d = seg.shape[0]
        int w = seg.shape[1]
        int h = seg.shape[2]
...
def computeGrayImage3dEnergy(float[:, :, :] img, int[:, :, :] seg):
    cdef:
        int nb_segments = np.max(seg) +1
        float[:] features = np.zeros(nb_segments, dtype=np.float32)
        int[:] count = np.zeros(nb_segments, dtype=np.int32)
...
def computeGrayImage3dVariance(float[:, :, :] img, int[:, :, :] seg, float[:] mean):
    cdef:
        int nb_segments = np.max(seg) +1
        float[:] features = np.zeros(nb_segments, dtype=np.float32)
        int[:] count = np.zeros(nb_segments, dtype=np.int32)
